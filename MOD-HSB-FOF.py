...
TRANSLATIONS = {
    "Português": {
        ...
        "select_exe": "Selecione o arquivo hl2.exe",
        "language": "Idioma",
    },
    "English": {
...
class InstallerGUI:
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.current_language = "English"  # Idioma padrão
        self.setup_window()
    ...
    def setup_window(self):
        ...
        # Define a geometria com a posição calculada
    ...
    def setup_gui(self):
        ...
        ttk.Label(lang_frame, text=TRANSLATIONS[self.current_language]["language"]).pack(side=tk.LEFT)
        ...
        self.status_label = ttk.Label(
            info_frame,
            text=TRANSLATIONS[self.current_language]["checking_install"],
            style="Info.TLabel"
        )
    ...
    def update_texts(self):
        ...
        if self.install_button["state"] == tk.NORMAL:
            self.status_label.config(text=TRANSLATIONS[self.current_language]["game_found"])
        else:
            self.status_label.config(text=TRANSLATIONS[self.current_language]["select_location"])
    ...
    def start_installation(self):
        if is_game_running():
            ...
            return  # Usa return ao invés de exit() para não fechar o programa
    ...
    def install_mod(self):
        try:
            ...
            self.status_label.config(text=TRANSLATIONS[self.current_language]["checking_files"])

            # Verifica arquivo do mod
            ...
            # Cria backup
            backup_name = f"client.dll.backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            backup_path = os.path.join(os.path.dirname(self.client_dll_path), backup_name)
            shutil.copy2(self.client_dll_path, backup_path)

            self.progress["value"] = 75
            self.log(TRANSLATIONS[self.current_language]["backup_created"].format(backup_name))

            # Instala o mod
            shutil.copy2(self.mod_dll_path, self.client_dll_path)

            self.progress["value"] = 100
            self.status_label.config(text=TRANSLATIONS[self.current_language]["install_complete"])
            ...
            self.log(f"❌ {str(e)}")
            self.status_label.config(text=TRANSLATIONS[self.current_language]["error_title"])
    ...
    # Adicionar este método à classe InstallerGUI
    def find_backups(self):
        """Encontra todos os arquivos de backup no diretório"""
        ...
        return sorted(backups, reverse=True)  # Mais recente primeiro
    ...
    def restore_backup(self):
        ...
        """Restaura um arquivo de backup selecionado"""
        ...
        # Criar uma nova janela para seleção do backup
        backup_window = tk.Toplevel(self.root)
        backup_window.title(TRANSLATIONS[self.current_language]["select_backup"])
        backup_window.geometry("400x300")

        # Lista de backups
        backup_list = tk.Listbox(backup_window, width=50)
        backup_list.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)

        for backup in backups:
            backup_list.insert(tk.END, backup)

        def do_restore():
            if not backup_list.curselection():
                return

            selected_backup = backup_list.get(backup_list.curselection())
            backup_path = os.path.join(os.path.dirname(self.client_dll_path), selected_backup)

            try:
                # Fazer backup do arquivo atual antes de restaurar
                current_backup = f"client.dll.pre_restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                current_backup_path = os.path.join(os.path.dirname(self.client_dll_path), current_backup)
                shutil.copy2(self.client_dll_path, current_backup_path)

                # Restaurar o backup selecionado
                shutil.copy2(backup_path, self.client_dll_path)

                self.log(f"✓ {TRANSLATIONS[self.current_language]['restore_success']}")
                messagebox.showinfo(
                    TRANSLATIONS[self.current_language]["success_title"],
